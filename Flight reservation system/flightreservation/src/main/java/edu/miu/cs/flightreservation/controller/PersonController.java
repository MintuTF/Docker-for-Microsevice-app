...
public class PersonController {
    ...
    @GetMapping ("/{id}")
    public ResponseEntity<?> getOnePerson(@PathVariable Long id, HttpServletRequest request){
       // System.out.println(username);
        System.out.println(request.getHeader("Authorization"));
        ResponseEntity<?> responseEntity=null;
        try {

            PersonResponse getUser = personServiceImp.getOnePersonById(id);
            if (getUser != null) {
    ...
    public ResponseEntity<?> createPerson(@RequestBody SignupRequest signupRequest){
        ...
        try {
              ...
              personServiceImp.createPerson(signupRequest);
                ...
                responseEntity=new ResponseEntity<>(signupRequest, HttpStatus.OK);
